...
void trace_back_path(const int &dist) {
    std::cout << "The shortest path from " << point(s) << " to " << point(t) << " is " << dist << "." << std::endl;
...
void log_u(const coord &u) {
    ...
    std::cout << "Choose cell " << point(u) << " with accumulated length " <<  u.pathcost << std::endl;
}

void log_v(const coord &v) {
    std::cout << "Cell " << point(v) << " with accumulated length " <<  v.pathcost << " is added into the queue." << std::endl;
}
